# Deploying Knotz Blackjack

The game is entirely client-side — the engine, the shoe, the strategy chart,
the counting trainer and the saved session all run in the browser, and nothing
needs a server except something to hand over the files. So `pnpm run build` produces a folder of static
...
Craps takes 8080 and Dragon's Shrine takes 8081, so all three can run at once
from the repository root's compose file.
...
shipped image contains no Node and no installed packages — just nginx, the built
game, and the 54-card PNG deck it deals from.
...
asks again; the second hand of a session touches the network for nothing.
...
It installs as *Blackjack*, with its own icon, and opens without Safari's address
bar because the manifest asks for `standalone`.

**Turn the phone sideways.** A blackjack layout is a wide, shallow arc — three
betting circles across the front edge with their side-bet boxes under them, and
in portrait every one of those is too small to hit accurately. Landscape fits
an iPhone exactly, with nothing cut off. Held upright, the game says so rather
than showing you a squeezed table.

If the phone cannot reach it, Windows Firewall is the usual reason: allow
Docker, or open TCP 8082 for private networks.
...
`TableGames/blackjack` when importing the project. Vercel reads `vercel.json` from
there.
...
```bash
docker build -t knotz-blackjack .
docker save knotz-blackjack | ssh you@your-server 'docker load'
```
...
```caddy
blackjack.example.com {
    reverse_proxy blackjack:80
}
```
...
```bash
curl -I http://localhost:8082/                      # 200, Cache-Control: no-cache
curl -I http://localhost:8082/manifest.webmanifest  # application/manifest+json
curl -I http://localhost:8082/nope                  # 404, not 200
curl    http://localhost:8082/healthz               # ok
curl -I -H 'Accept-Encoding: gzip' \
     http://localhost:8082/_next/static/chunks/*.js # Content-Encoding: gzip
```
