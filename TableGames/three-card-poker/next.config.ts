...
/*
 * Knotz Three Card Poker is a client-side game: no API routes, no server
 * data, nothing that has to be rendered per-request. So it builds to a folder
 * of static files, which is what lets a plain nginx container serve it and
 * what lets it be dropped on any static host without a Node process running
 * anywhere.
 *
...
const nextConfig: NextConfig = {
  ...
  // `/` and any future route without needing rewrite rules.
  trailingSlash: true,
