...
Port **8081**, not 8080, so this and [the craps table](../../TableGames/craps)
...
Cinzel, Inter and Orbitron at build time and self-hosts them in the output.
That is what stops the running game from calling out to Google, but it does
mean an offline machine cannot build the image.
...
network can reach it at `http://<this machine's LAN IP>:8081`. On the phone,
open that address, then **Share → Add to Home Screen**. It installs as
*Dragon's Shrine*, with its own icon, and opens without the browser's address
bar because the manifest asks for `standalone`.

Unlike the craps table, this one does not care which way up the phone is. A
slot cabinet is a tall object: portrait is the better orientation and the
default, and landscape re-flows the chrome around the reels rather than
refusing to play.

If the phone cannot reach it, a firewall is the usual reason: allow Docker, or
open TCP 8081 for private networks.
...
than forwarding a port at home. In rough order of how little work they are:
...
because every push then redeploys on its own. The one setting that matters:
this game is not at the repository root, so set **Root Directory** to
`SlotsGames/DragonsShrine` when importing the project. Vercel reads
...
```bash
docker build -t dragons-shrine .
docker save dragons-shrine | ssh you@your-server 'docker load'
```
...
```caddy
shrine.example.com {
    reverse_proxy dragons-shrine:80
}
```
...
`gzip_static`. Everything here is code, fonts and SVG — the artwork is drawn at
runtime rather than shipped as images — so it compresses hard.
...
```bash
curl -I http://localhost:8081/                      # 200, Cache-Control: no-cache
curl -I http://localhost:8081/manifest.webmanifest  # application/manifest+json
curl -I http://localhost:8081/nope                  # 404, not 200
curl    http://localhost:8081/healthz               # ok
curl -I -H 'Accept-Encoding: gzip' \
     http://localhost:8081/_next/static/chunks/*.js # Content-Encoding: gzip
```
...
- **A persistent progressive jackpot.** The four jackpots are fixed multiples of
  the stake, which is the honest thing to do without a server. A jackpot that
  genuinely grows across sessions and players needs one.
